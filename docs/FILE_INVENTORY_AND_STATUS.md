...
| [`app/services/live_experiment_service.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/services/live_experiment_service.py) | **`[INTEGRATED]`** | Lightweight coordinator facade orchestrating order creation, webhook processing, and lift calculation. | High-level coordinator delegating to `experiment_order_creator.py`, `webhook_payment_processor.py`, and `experiment_metrics_calculator.py`. |
| [`app/services/experiment_order_creator.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/services/experiment_order_creator.py) | **`[INTEGRATED]`** | Creates Razorpay Test Orders for treatment cohorts with structured notes and rate-limit pacing. | Calls `razorpay_client.create_order` with custom notes (`campaign_id`, `customer_id`, `variant`) and seeds initial `experiment_assignments`. |
| [`app/services/webhook_payment_processor.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/services/webhook_payment_processor.py) | **`[INTEGRATED]`** | Processes incoming Razorpay webhooks, deduplicates transactions, and converts experiment assignments. | Verifies signatures, matches orders via notes/order ID, commits PostgreSQL payments, and marks `experiment_assignments.is_converted = True`. |
| [`app/services/experiment_metrics_calculator.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/services/experiment_metrics_calculator.py) | **`[INTEGRATED]`** | Computes mathematical conversion rates, relative lift %, and net incremental GMV from experiment assignments. | Aggregates treatment vs control conversion rows, invokes `ExperimentAgent`, and vectorizes outcome memories into ChromaDB. |
| [`app/services/metrics_service.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/services/metrics_service.py) | **`[INTEGRATED]`** | Prometheus-compatible metrics registry tracking request counts, agent durations, and payment operations. | Exposes counters, gauges, and histograms at `/metrics` (JSON) and `/metrics/prometheus` (Prometheus text format). |
| [`app/services/agent_performance_tracker.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/services/agent_performance_tracker.py) | **`[INTEGRATED]`** | Tracks and aggregates runtime performance, latencies, and success/failure rates per agent. | Records start/end timestamps, rolling min/max/avg latency in ms, and success rates for every agent execution. |
| [`app/services/cache_service.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/services/cache_service.py) | **`[INTEGRATED]`** | In-memory caching service for hot-path queries (Customer 360, RFM segments, opportunity scans). | High-speed cache with TTL expiration, hit/miss tracking, function decorators, and merchant cache invalidation. |
...
| [`app/services/llm_provider_service.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/services/llm_provider_service.py) | **`[INTEGRATED]`** | 3-tier benchmarked multi-provider cascade across NVIDIA NIM, OpenRouter, Groq, and Mistral. | Manages model chains per provider with automatic model fallbacks, token budgeting (`chat: 600`), and SSE token streaming. |
| [`app/services/llm_service.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/services/llm_service.py) | **`[INTEGRATED]`** | Connects to AI models via `LLMProviderService` with tool-calling and automatic deterministic fallback. | Supports `call_with_tools`, `stream_chat_with_merchant`, personalized copy, and reasoning trace extraction. |
...
| File Path | Status | Production Specification | Current Implementation |
|:---|:---|:---|:---|
| [`app/intelligence/distribution_thresholds.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/intelligence/distribution_thresholds.py) | **`[INTEGRATED]`** | Statistical percentile engine extracting empirical $P_{90}$ spend, $P_{75}$ frequency, $P_{50}$ recency, and payment baselines. | Computes empirical distribution quantiles from customer population to eliminate hardcoded magic numbers across all intelligence models. |
| [`app/intelligence/customer_segmentation.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/intelligence/customer_segmentation.py) | **`[INTEGRATED]`** | Classifies customers using distribution-aware RFM composite scoring into 6 distinct behavioral cohorts. | Weighted RFM scoring mapping customers into `VIP Active`, `VIP Dormant`, `Loyal`, `Loyal At Risk`, `New`, and `Standard` anchored to population quantiles. |
| [`app/intelligence/churn_predictor.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/intelligence/churn_predictor.py) | **`[INTEGRATED]`** | Continuous 3-factor composite churn model: 50% recency CDF, 30% frequency decay, 20% spend decline. | Analyzes historical order intervals and spend trajectories with smooth continuous functions to yield a 0.0-1.0 risk score. |
| [`app/intelligence/clv_estimator.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/intelligence/clv_estimator.py) | **`[INTEGRATED]`** | 12-month forward predictive CLV combining continuous logarithmic frequency scaling and churn discount. | Computes projected CLV with smooth frequency multiplier curve clamped above historical spend. |
...
| [`app/intelligence/payment_method_analyzer.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/intelligence/payment_method_analyzer.py) | **`[INTEGRATED]`** | Benchmarks payment method success rates against the merchant's empirical baseline and quantifies lost GMV. | Identifies underperforming channels against empirical 75th-percentile benchmark and computes recoverable revenue. |
| [`app/intelligence/opportunity_detector.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/intelligence/opportunity_detector.py) | **`[INTEGRATED]`** | Scans merchant intelligence for 5 distinct opportunity types with distribution-aware thresholds and confidence. | Computes Dormant VIP Recovery, Cross-Sell Affinity, Payment Optimization, Proactive Churn Intervention, and Tiered Basket Builder auto-scaling with store AOV. |
...
| [`app/agents/agent_consensus.py`](file:///c:/Users/ffmou/Desktop/razorpay/app/agents/agent_consensus.py) | **`[INTEGRATED]`** | Builds multi-agent consensus across competing recommendations using confidence weighting and voting. | Implements confidence-weighted, majority voting, and unanimous consensus builders with agreement metrics. |
